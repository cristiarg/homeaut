...
public:
    ...
    static void resetA() {
        ...
        OCR1A = 0;
        // TCNT1 = 0; // adding this does not make a difference
        //uint8_t bbb = TCCR1B;
        //Serial.println(bbb, 2);
        //TCCR1B = 0; // this makes a difference, but it actually means that comparer A cannot work with recurrence independent of comparer B
        //TCNT1 = 0; // this one and the one above together are needed
        //bbb = TCCR1B;
        //Serial.println(bbb, 2);
        TIFR1 &= ~(1 << OCF1A);
    }
